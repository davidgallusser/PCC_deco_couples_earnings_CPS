##############################################################
## 8 - CPS - deco quantile plots 1976 - 2016 - conditional on education
##############################################################

years <- c(1976,2016)
setwd(paste0(wd,datafiles_d))
#load(paste0("copula_decomposition_main_",years[1],"-",years[2],"_new.rda"))
...
plotdf1 <- VCopObj_cond$VCdeco$composition_effect$quants_deco
plotdf2 <- VCopObj_cond$VCdeco$wage_structure_effect$quants_deco
plotdf1$effect <- "Education"
plotdf2$effect <- "Other"
...
if(is.null(VCopObj_cond$VCdeco_se$aggregate)==FALSE){
  plotdf2a <- VCopObj_cond$VCdeco_se$composition_effect$quants_deco_se
  plotdf2b <- VCopObj_cond$VCdeco_se$wage_structure_effect$quants_deco_se
  ...
  plotdf2 <- melt(plotdf2, id.vars=names(plotdf2)[1:5],
                  ...
                  variable.name = "tau",
                  value.name = "se")
  plotdfa <- subset(plotdf,effect =="Education")
  plotdf2a <- subset(plotdf2,effect =="Education")
  plotdfb <- subset(plotdf,effect !="Education")
  ...
  plotdfa$t <- VCopObj_cond$VCdeco_se$composition_effect$quants_deco_critval[match(plotdfa[,1],VCopObj_cond$VCdeco_se$composition_effect$quants_deco_critval$names1),"t"]
  plotdfb$t <- VCopObj_cond$VCdeco_se$wage_structure_effect$quants_deco_critval[match(plotdfb[,1],VCopObj_cond$VCdeco_se$wage_structure_effect$quants_deco_critval$names1),"t"]
...
fn <- paste0(pfolder,"deco_quantiles_aggregate_education",ptype)
...
fn <- paste0(pfolder,"deco_quantiles_detailed_marginals1_education",ptype)
ggsave(fn, units=punits, scale=pscale, width=pwidth, height=pheight)
...
fn <- paste0(pfolder,"deco_quantiles_detailed_marginals2_education",ptype)
...
fn <- paste0(pfolder,"deco_quantiles_detailed_copula_education",ptype)
ggsave(fn, units=punits, scale=pscale, width=pwidth, height=pheight)
