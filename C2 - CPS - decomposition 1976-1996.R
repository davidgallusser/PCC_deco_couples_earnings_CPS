##############################################################
## C2 CPS decomposition 1976-1996
##############################################################
...
impute <- list(list(sf <- LFP_f ~ NCHLT5_f + NCHILD518_f + asinh(abs(INCIDR_hh)) + asinh(INCWAGE_m) + EDUC2_f + AGE_f + I(AGE_f^2/100) + METRO_f  + REGION_f,
                    wf <- log(WAGE_f) ~ EDUC_f*(EXP_f + I(EXP_f^2/100)) + METRO_f + REGION_f + RACE_f + HISPAN_f,
                    weights="ASECWT_f"),
               list(sm <- LFP_m ~ NCHLT5_m + NCHILD518_m + asinh(abs(INCIDR_hh)) + asinh(INCWAGE_f) + EDUC2_m + AGE_m + I(AGE_m^2/100) + METRO_m  + REGION_m,
                    wm <- log(WAGE_m) ~ EDUC_m*(EXP_m + I(EXP_m^2/100)) + METRO_m + REGION_m + RACE_m + HISPAN_m ,
                    weights="ASECWT_m"))
...
# # Check overall fit
# VC_deco_qqplot(VCopObj,formula=NULL, type=1, logscale=TRUE, group=c("1975","1996"))
# VC_deco_qqplot(VCopObj,formula=NULL, type=2, logscale=FALSE)
# VC_deco_qqplot(VCopObj,formula=NULL, type=4, logscale=TRUE, tau=5:99/100)
# VC_deco_qqplot(VCopObj,formula=NULL, type=4, logscale=FALSE, tau=5:99/100)
# 
...
# compare_plots(plotlist,edge=1, groups=c(1976,1996))
# compare_plots(plotlist,edge=2, groups=c(1976,1996))
# compare_plots(plotlist,edge=3, groups=c(1976,1996))
# compare_plots(plotlist,edge=4, groups=c(1976,1996))
# compare_plots(plotlist,edge=5, groups=c(1976,1996))
# compare_plots(plotlist,edge=6, groups=c(1976,1996))
# 
