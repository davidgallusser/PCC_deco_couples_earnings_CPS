...
##########################################################################
VC_deco_bs <- function(VCopObj, deco_res, it=100, 
                       ncore=1, psu=NULL, stratum=NULL, alpha=0.05){
  
  # Store simulation parameter
  deco_sim_par <- deco_res$par

  # Resample 
  s <- VC_resample(VCopObj, it=it, psu=psu, stratum=stratum)

  # Restimate (parallelized if requested)
  if(ncore==1){  
    #res <- lapply(s, function(x) VC_deco_for_bs(VCopObj,x))
    res <- pblapply(s, function(x) VC_deco_for_bs(VCopObj,x,deco_sim_par))
    ...
    clusterExport(cl=cl, 
                  varlist=c("VCopObj","deco_sim_par",foos),
                  envir=environment()
    )

    # Run parLapply on 
    #res <- parLapply(cl, s, function(x) VC_deco_for_bs(VCopObj,x))
    res <- pblapply(s, function(x) VC_deco_for_bs(VCopObj,x,deco_sim_par), cl=cl)
  ...
  quants_deco_est <- deco_res$quants_deco
  ...
  quants_sim_vs_obs_est_1 <- t(deco_res$quants_sim_vs_obs)[2,]
  quants_sim_vs_obs_est_0 <- t(deco_res$quants_sim_vs_obs)[3,]
  quants_sim_vs_obs_est_dd <- t(deco_res$quants_sim_vs_obs)[4,]
  ...
  ### P-values of Kolmogorov-Smirnov and Cramér-von-Mises-tests
  # Diference between simulated and observed quantiles
  ...
  # Return results
  return(list(quants_deco_se=quants_deco_se,
...
VC_resample <- function(VCopObj,it,psu,stratum){
  
  n0 <- nrow(VCopObj$data0)
  n1 <- nrow(VCopObj$data1)
  ...
  if(is.null(stratum)){
    ...
    stratum <- list(VCopObj$data0[,stratum],VCopObj$data1[,stratum])
  }
  if(is.null(psu)){
    psu <- list(1:length(w0),1:length(w1))
    }else{
    psu <- list(VCopObj$data0[,psu],VCopObj$data1[,psu])
  }
...
VC_deco_for_bs <- function(VCopObj,s,deco_sim_par){
  
  # Bootstrap sample
  VCopObj$data0 <- VCopObj$data0[s[[1]],]  
  VCopObj$data1 <- VCopObj$data1[s[[2]],] 
  
  VCopObj$u0 <- apply(VCopObj$data0[,2:5], 2, function(x) rank.wt(x,VCopObj$data0[,"(weights)"])) #VCopObj$u0[s[[1]],]  
  VCopObj$u1 <- apply(VCopObj$data1[,2:5], 2, function(x) rank.wt(x,VCopObj$data1[,"(weights)"])) #VCopObj$u1[s[[2]],] 
  
  ## Re-estimate copula with adjusted weights
  VCopObj$copula0 <- kdeVineCop(VCopObj$u0, w=VCopObj$data0[,"(weights)"]*s[[3]], 
                                order= VCopObj$order, type=VCopObj$type, print_info=FALSE)
  VCopObj$copula1 <- kdeVineCop(VCopObj$u1, w=VCopObj$data1[,"(weights)"]*s[[4]], 
                                order= VCopObj$order, type=VCopObj$type, print_info=FALSE)
  
  ## Re-simulate data and perform decomposition
  deco_res <- VC_deco_sim(VCopObj, print_info=FALSE, export_usim=FALSE,
                          n=deco_sim_par$n,
                          reference = deco_sim_par$reference,
                          multiply = deco_sim_par$multiply,
                          quasi = deco_sim_par$quasi,
                          copula_detail = deco_sim_par$copula_detail
                          )
  return(deco_res)
...
##########################################################################
# Compute p-values of Kolmogorov-Smirnov- and Cramér-von-Mises-type tests 
