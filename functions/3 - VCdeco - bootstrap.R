##########################################################################
#### 3 - VCdeco - bootstrap inference for pair-copula decompositions
##########################################################################

#### The following code contains bootstrap function for the pair-copula 
#### decomposition
require(parallel)
require(pbapply)
...
# by inverting of bootstrapped Kolmogorov-Smirnov test limiting distribution
# e.g. distribution of standardized differences between estimated and bootstrapped QTE
# Critical value is than alpha-quantile of bootstrapped distribution
# (slightly adapted code from Chen et al. (2017), Counterfactual::VarianceEval())
...
##########################################################################
# Compute p-values of Kolmogorov-Smirnov- and Cramèr-von-Mises-type tests 
# by estimating bootstrapped KS and CMS test limiting distribution 
# e.g. distribution of standardized differences between (1) estimated quantile difference 
# of observed and simulated distribution and (2) bootstrapped quantile differences
# of observed and simulated distribution.
# For instance, p-value of KS test is then share of differences-in-differences at supremum 
# that are larger than difference between observed quantiles and simulated quantiles of estimation
# at supremum. 
# Intuition: If share is large, we consider supremum difference between observed and simulated as random. 
# (slightly adapted code from Chen et al. (2017), Counterfactual::TestingEval()

KS_CMS_test <- function(boot_test_numerator, 
                        obs_test_numerator,
                        variable_for_variance){
  # boot_test_numerator = all bootstrapped differences between quantiles of observed and simulated distributions 
  # obs_test_numerator = estimated differences between quantiles of observed and simulated distributions
  # variable_for_variance = boot_test_numerator
  # Vuqf: vector with variance of differences by quantile
  Vuqf <- robust_bs_se(variable_for_variance)^2
  Vuqf = Vuqf + 1e-09
  reps <- nrow(boot_test_numerator)
  nqs = ncol(boot_test_numerator)
  # Kuqf: 'Standardized' boot_test_numerator (i.e. boot_test_numerator/ s.e. of boot_test_numerator by quantile)
  boot_test_numerator <- boot_test_numerator - kronecker(matrix(obs_test_numerator, 1, nqs), matrix(1, reps, 1))
  Kuqf = sqrt(boot_test_numerator^2/
                kronecker(matrix(Vuqf, 1, nqs), matrix(1, reps, 1)))
  # Kmaxuqf: Maximal 'standardized' difference for every bs iteration
  Kmaxuqf = apply(Kuqf, 1, max)
  # KSstat: Maximum/sup of std. estimated difference between observed and simulated dist. 
  KSstat = max(sqrt(obs_test_numerator^2/Vuqf))
  Kuqf2 = Kuqf^2
  Kmeanuqf = rowMeans(Kuqf2)
  CMSstat = mean(obs_test_numerator^2/Vuqf)
  # p-values: Share of bootstrapped statistic that are larger than estimated statistic
  # e.g. share of bootstrapped maximal quantile differences than estimated maximal quantile difference
